...
def generate_benchmark_cases(dims):
    ...
    cases = [
        ...
        (
            "Div",
            f"({py_sum}) / ({py_sum} + 0.01)",
            f"({cosy_sum}) / ({cosy_sum} + 0.01)",
        ),
...
def run_ops_benchmark(engine, args):
    ...
    for name, mtf_expr, cosy_expr in ops:
        ...
        try:
            with TimeLimit(args.timeout):
                ...
                if args.memory:
                    ...
                    _, mem_py = measure_memory(
                        engine.run_sandalwood, mtf_expr, "python", args.iters
                    )
                    _, mem_sc = measure_memory(
                        engine.run_sandalwood, mtf_expr, "cosy", args.iters
                    )
        except TimeoutError:
        ...
        row = {
            ...
            "Speedup (vs Python)": engine.format_speedup(
                t_py / t_sc if t_sc > 0 else 0
            ),
        }
    ...
    for name, mtf_expr, cosy_expr in cases:
        ...
        if args.mode == "cosy_raw":
            ...
            try:
                with TimeLimit(args.timeout):
                    # Timing run first
                    c_py, t_py, s_py = engine.run_sandalwood(
                        mtf_expr, "python", args.iters
                    )
                    if args.memory:
                        # Memory run second
                        _, mem_py = measure_memory(
        ...
        c_raw, t_raw, s_raw = engine.run_raw_cosy(
            name, cosy_expr, args.iters, timeout=args.timeout
        )
        ...
        if pd.isna(t_sc) and pd.isna(t_raw):
            print(
                f"Skipping {name} - Both S-COSY and Raw COSY failed/timed out",
                file=sys.stderr,
            )
            continue
        ...
        rmse_py = (
            engine.calculate_rmse(c_py, c_raw)
            if not pd.isna(t_py) and not pd.isna(t_raw)
            else np.nan
        )
        rmse_sc = (
            engine.calculate_rmse(c_sc, c_raw)
            if not pd.isna(t_sc) and not pd.isna(t_raw)
            else np.nan
        )

        speedup_py = (
            t_py / t_sc
            if (not pd.isna(t_py) and not pd.isna(t_sc) and t_sc > 0)
            else np.nan
        )

        def safe_fmt(val):
            return f"{val:.2e}" if not pd.isna(val) else "N/A"
        ...
        disp_row = {
            ...
            if not pd.isna(t_raw)
            else "N/A",
            "RMSE (Py)": safe_fmt(rmse_py),
            "RMSE (SC)": safe_fmt(rmse_sc),
            "Speedup": engine.format_speedup(speedup_py)
            if not pd.isna(speedup_py)
            else "N/A",
        }
    ...
    results = [
        {
            ...
            "Speedup": engine.format_speedup(
                t_serial_est / t_batch if t_batch > 0 else 0
            ),
        }
    ]
...
def run_full_benchmark(args):
        ...
        for o in orders:
            ...
            try:
                res_raw = subprocess.run(
                    cmd_raw, capture_output=True, text=True, check=True
                )
                out = res_raw.stdout
                ...
                for item in raw_data:
                    ...
                    if speedup_py is None:
                        speedup_py = np.nan

                    # Recompute raw speedup if needed, or trust child
                    speedup_raw = (
                        t_raw / t_sc
                        if (t_sc > 0 and not np.isnan(t_sc) and not np.isnan(t_raw))
                        else np.nan
                    )
    ...
    if args.mode == "full_cosy":
        report_title = "COSY Backend Performance Report"

    engine = BenchmarkEngine(10, 6)
    plots = engine.generate_plots(full_results)
    report_path = engine.generate_html_report(
        full_results, plots, method_info={"iterations": iters, "title": report_title}
    )
...
def main():
    ...
    parser.add_argument(
        "--timeout", type=int, default=30, help="Benchmark timeout in seconds"
    )

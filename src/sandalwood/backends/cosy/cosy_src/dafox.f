...
*     **********************
*
*     THIS SUBROUTINE RETURNS THE ELAPSED CPU TIME (SEC).
*
...
*
      REAL TIMS,TIM(2)
*
*     CPU_TIME() IS DEFINED BY THE FORTRAN 95 STANDARD TO RETURN
*     "A PROCESSOR-DEPENDENT APPROXIMATION TO THE PROCESSOR TIME".
*     HOWEVER, "THE EXACT DEFINITION OF TIME IS LEFT IMPRECISE BECAUSE OF
*     THE VARIABILITY IN WHAT DIFFERENT PROCESSORS ARE ABLE TO PROVIDE."
*
*     WHAT IS REPORTED BY CPU_TIME OR BY ETIME VARIES DEPENDING ON
*     THE PLATFORM, THE COMPILER, AND THE SYSTEM CONDITION.
*     THE REPORTED VALUES MAY FLUCTUATE AS MUCH AS 50% TO 200%.   - MAY 2023
*
*     IF ETIME() WROKS BETTER FOR YOUR SYSTEM, UNCOMMENT THE ETIME() LINE,
*     AND COMMENT OUT THE CPU_TIME CALLING LINE.
*
      CALL CPU_TIME(TIMS)
C     TIMS = ETIME(TIM)
*
...
*
*     SLEEPQQ IS INTEL FORTRAN COMPILER SPECIFIC AND PLATFORM INDEPENDENT.
*     SLEEP IS NOT COMPILER SPECIFIC AND PLATFORM INDEPENDENT, BUT
*     ONLY DOES FULL SECONDS, NOT MILLISECONDS. IF AVAILABLE, USE SLEEPQQ.
*
*     IF SLEEPQQ IS NOT AVAILABLE, COMMENT OUT THE SLEEPQQ(..) CALLING LINE,
*     AND UNCOMMENT THE SLEEP(..) CALLING LINE BELOW.
*
*     THIS COMMENT/UNCOMMENT CHANGE CAN BE MADE BY HAND-EDITING.
*     ALTERNATIVELY, THE PROGRAM VERSION CAN BE USED. PLEASE REFER TO
*     SECTION 1.5.4. "Conversion of a Source File Using VERSION"
...
*
      IUNITS = IUNIT
      IF (IUNIT.LT.0) CALL GRIUF(IUNIT,IUNITS,IUNITA)
*
      IF(IUNITS.LE.-201.AND.IUNITS.GE.-210) THEN
         CALL GUIPRI(INA,IUNITS,IWRT,NWRT)
      ELSE
         CALL FOXPRIO(INA,IUNITS)
      ENDIF
*
      RETURN
      END
...
*      * UPDATE THE DELAYED READ STRUCTURES TO MATCH CLOSED WINDOW
         ...
         ENDIF
*
*   * OUTPUT GRAPHICS TO THE GUI DIRECTLY
      ELSEIF(LCONS.EQ.0.AND.NTYP(INA).EQ.NGR) THEN
*      * OUTPUT THE GUI COMMAND IF ANY WITH THE ARGUMENTS TO THE GUI
         IF(LA.GT.0) THEN
            CALL GUICMD(SNR//' '//A(1:LA))
            LA = 0
         ENDIF
*      * SET UP GUI FOR GRPRI
         CALL GUICMD(SNR//' canvas')
         CALL GUICMD(SNR//' redirect')
         CALL GRPRI(INA,IUNIT)
         CALL GUICMD('0 redirect')
*      * AND STORE IN DELAYED READ ARRAY
*      * HOWEVER, DELAYED READ FOR GUI GR NOT IMPLEMENTED
*        CALL GUICMD(SNR//' newline')
*        NGVAR(IWND,1) = NGVAR(IWND,1)+1
*        IGVAR(IWND,NGVAR(IWND,1),1) = -1
*
...
*     ******************************
*
*     THIS SUBROUTINE SETS GUI FIELD INB IN GUI WINDOW UNIT INA
*     TO THE VALUE IN INC.
*     THIS SUBROUTINE ACCEPTS GRAPHICS FOR INC, JUST OUTPUTTING IT.
*
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
*
*-----MEMORY MANAGEMENT -----------------------------------------------------
...
*----------------------------------------------------------------------------
*
      CHARACTER STR*4096,STRE*(2*4096+2),SNR*3,GNR*3
*
      IF(NTYP(INA).NE.NRE) CALL FOXNTY(INA)
...
*
      ...
      ELSE
         IB = NINT(CC(NBEG(INB)))
         IF(IB.LT.1) THEN
            PRINT*,'$$$ ERROR IN GUISET, ILLEGAL GUI ELEMENT NUMBER: ',
     *             IB
            CALL FOXDEB
         ENDIF
         WRITE(GNR,'(I3)') IB
...
*
      IF(IU.EQ.0) RETURN
*
C      PRINT*,'   ARRAY SETUP DONE, BEGIN PRINTING'
*
      IOUT = 32
...
*
      IF(INTINI.EQ.-1) THEN
*
*        THE FOLLOWING NUMBERS FOR MINEX0, RINMI0 ARE FROM MS WIN PC
*        USING INTEL IFORT WITH THE COMPILER OPTION -fp-model strict
*        AND ALSO USING GNU GFORTRAN. 2023
*
         MINEX0 = 1022
         RINMI0 = 0.44501477170144037543D-307
*
         INTINI = 1
         MANTIS = 1
         EPSI(MANTIS) = 1.D0
...
*
 50      IF((A*RINUP(1).LE.A).OR.(B*RINUP(1).LE.B).OR.
     ...
     *      (B*RINDN(1).GE.B).OR.(A*RINDN(1).GE.A)) THEN
         ...
         ENDIF
*
C        IF(FLAG FOR THE FOLLOWING TASK IS ON) THEN
C
C           PRINT*,'$$$ NOTE: This code produces deliberate floating '//
C    *       'point underflows, for which'
C           PRINT*,'    some compilers may issue warning messages, '//
C    *       'which can be safely ignored.'
C
C           MINEXP = 1
C           EPSI(MINEXP) = 1.D0
C60         IF(EPSI(MINEXP)/2.D0.GT.0.D0) THEN
C              EPSI(MINEXP+1) = EPSI(MINEXP)/2.D0
C              MINEXP = MINEXP+1
C              GOTO 60
C           ENDIF
C
C           MINEXP = MINEXP-MANTIS
C           RINMIN = EPSI(MINEXP)/RINDN(1)
C
C           MORE TASKS:
C              COMPARE MINEXP, RINMIN WITH MINEX0, RINMI0.
C              ISSUE AN APPROPRIATE MESSAGE.
C              HAVE A TEST SUITE, AND REPORT.
C        ENDIF
*
         MINEXP = MINEX0
         RINMIN = RINMI0
...
*        WRITE(17,*) '*** EPSM,EPS'
*        WRITE(17,'(G27.20E3)') EPSM
*        WRITE(17,'(G27.20E3)') EPS
*        CLOSE(17)
*
      ENDIF
*
...
*
      DO 10 I=IA,NEND(INA)
      CCC = CC(I)*CC(IB)
      IF(ABS(CCC).LT.1.D-130) CCC = 0.D0
      CC(IC) = CCC
      IB = IB+1
 10   IC = IC+1
*

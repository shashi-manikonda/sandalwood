...
def test_erf(implementation):
    ...
    assert np.isclose(val_deriv, expected_deriv), f"Expected {expected_deriv}, got {val_deriv}"
...
def test_estimate_stability(implementation):
    ...
    if implementation == "python":
        with pytest.raises(NotImplementedError, match="estimate_stability is only available for the COSY backend"):

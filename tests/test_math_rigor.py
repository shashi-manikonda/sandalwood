...
def test_calculus_trig_derivatives(implementation):
    ...
    assert np.allclose(
        diff_s.truncate(max_order - 1).get_max_coefficient(), 0.0, atol=1e-12
    ), "d(sin)/dx != cos (truncated)"

    # d/dx cos(x) should be -sin(x)
    dc_dx = c.deriv(1)
    diff_c = dc_dx + s
    assert np.allclose(
        diff_c.truncate(max_order - 1).get_max_coefficient(), 0.0, atol=1e-12
    ), "d(cos)/dx != -sin (truncated)"

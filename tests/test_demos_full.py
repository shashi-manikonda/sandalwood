...
def _test_notebook(src_path, dst_path, backend):
    ...
    if result.returncode != 0:
        pytest.fail(f"Notebook {os.path.basename(src_path)} failed with backend {backend}:\n{result.stderr}")
...
def _test_script(src_path, dst_path, backend):
    ...
    result = subprocess.run(cmd, env=env, capture_output=True, text=True)
    if result.returncode != 0:
        pytest.fail(f"Script {os.path.basename(src_path)} failed with backend {backend}:\n{result.stderr}")
